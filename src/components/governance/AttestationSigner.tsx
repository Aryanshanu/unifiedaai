...
export function AttestationSigner({ modelId, modelName, frameworkId }: AttestationSignerProps) {
  ...
  const generateHash = (content: string): string => {
    // Simulated SHA-256 hash - NOT cryptographically secure
    let hash = 0;
  ...
  const generateMinisignSignature = (): string => {
    // Simulated minisign-style signature - NOT cryptographically secure
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          document_url: `data:application/json,${encodeURIComponent(JSON.stringify({
            ...
            timestamp,
            signed_by: signedBy,
            disclaimer: "This signature is simulated for demonstration purposes and is not cryptographically secure."
          }))}`,
        })
        .select()
        .single();
...
<body>
  ...
  <div class="warning">
    <strong>DEMONSTRATION ONLY:</strong> This attestation uses simulated signatures for demonstration purposes. 
  ...
  <div class="signature-box">
    <div class="section">
      <div class="label">Simulated Signature (minisign-style)</div>
      <div class="value">${signedAttestation.signature}</div>
    </div>
    
    <div class="section">
      <div class="label">Simulated Hash</div>
          ...
          <div className="bg-warning/10 border border-warning/30 rounded-lg p-3 flex items-start gap-3">
            ...
            <div className="text-sm">
              ...
              <span className="text-muted-foreground">
                Signatures are simulated and not cryptographically secure. For production use, integrate with a real signing service.
              </span>
            </div>
          </div>
            ...
            <>
              ...
              <div className="bg-secondary/30 rounded-lg p-4 text-sm text-muted-foreground">
                <p className="flex items-center gap-2">
                  ...
                  This will generate a simulated signature for demonstration purposes.
                </p>
              </div>
            ...
            <>
              <div className="bg-success/10 border border-success/20 rounded-lg p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Simulated signature generated (not production-grade)</p>
                </div>
              </div>

              <div className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground">SIMULATED SIGNATURE</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground">SIMULATED HASH</Label>
                  <div className="flex items-center gap-2">
                    <code className="flex-1 text-sm bg-secondary p-3 rounded-lg font-mono overflow-x-auto">
